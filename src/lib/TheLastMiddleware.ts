...
export function TheLastMiddleware(config: TheLastMiddlewareConfig) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
       ...
       */
      const reqRai = findRequestRai(
        req,
        Array.from(RaisMap.values()).map((r) => ({
          path: r.path,
          method: r.method,
          rai: r.rai,
        }))
      ...
      const user_roles: string[] = req.user?.roles || [guestRole];
      ...
      if (!isAuthorized) {
        ...
        // set status code 403
        res.status(
          user_roles.length === 1 && user_roles[0] === guestRole ? 401 : 403
        );
        return next(
          new errors.RequestError(
            `User is not authorized to access the RAI: ${reqRai}`,
            {
              code: "UNAUTHORIZED",
              route: path,
              method,
