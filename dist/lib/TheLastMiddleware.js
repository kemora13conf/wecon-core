...
import { findRequestRai } from "../middlewares/findRequestRai";
 ...
 */
export function TheLastMiddleware(config) {
    ...
    return async (req, res, next) => {
        ...
        try {
             ...
             */
            const reqRai = findRequestRai(req, Array.from(RaisMap.values()).map((r) => ({
                path: r.path,
                method: r.method,
                rai: r.rai,
            })));
            ...
            if (!router) {
                ...
                // set status code 404
                res.status(404);
                return next(new errors.RequestError(`No RAI found for the request URL: ${path} with method: ${method}`, {
                    code: "RAI_NOT_FOUND",
                    route: path,
                    method,
                }));
            ...
            if (!isAuthorized) {
                ...
                return next(new errors.RequestError(`User is not authorized to access the RAI: ${reqRai}`, {
                    code: "UNAUTHORIZED",
                    route: path,
                    method,
