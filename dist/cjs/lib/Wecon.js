...
class Wecon {
    ...
    routes(routes) {
        ...
        if (!(routes instanceof Routes_1.default)) {
    ...
    build() {
        ...
        this._raiMatcher = new RaiMatcher_1.default(routesList);
        // 2. Initialize ONE Master Router (For Performance)
        this._masterRouter = (0, express_1.Router)({ mergeParams: true });
        /**
                ...
                if (param.validate) {
                    this._masterRouter.param(param.path, (req, res, next, val) => {
                        if (!param.validateValue(val)) {
                            ...
                            return next(new errors_1.default.RequestError("Invalid Parameter", {
                                code: "INVALID_PARAM",
                            }));
                        }
                        next();
    ...
    compareRoutes(a, b) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (segA === undefined)
                return 1; // Short paths usually generic, push down? No, usually /users vs /users/list.
        ...
        return async (req, res, next) => {
            try {
                ...
                if (!reqRai) {
                    ...
                    res.status(404);
                    return next(new errors_1.default.RequestError(this._dev?.helpfulErrors
                ...
                const user_roles = user?.roles || [this._guestRole];
                if (!route.isAuthorized(user_roles)) {
                    res.status(user_roles.includes(this._guestRole) && user_roles.length === 1
                        ? 401
                        : 403);
                    return next(new errors_1.default.RequestError(this._dev?.helpfulErrors
     ...
     */
    async generatePostman() {
        if (!this._postman) {
            throw new Error("Postman configuration not provided. Call postman() before generatePostman()");
        }
        if (!this._built) {
            throw new Error("Cannot generate Postman collection before build() is called");
        }
        if (!this._routes) {
            throw new Error("Routes not configured. Cannot generate Postman collection.");
        }
        try {
            const { collection, environment } = await PostmanGenerator_1.default.generateFromWecon({
