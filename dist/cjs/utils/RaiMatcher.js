...
class RaiMatcher {
    ...
    initialize(raisList) {
        for (const route of raisList) {
            ...
            const cachedRoute = {
                matcher: (0, path_to_regexp_1.match)(route.path, { decode: decodeURIComponent }),
                rai: route.rai,
    ...
    findRai(path, method) {
        ...
        if (!methodRoutes) {
            throw new errors_1.default.RequestError(`No RAI found for the request URL: ${path} with method: ${method}`, {
                code: "RAI_NOT_FOUND",
                route: path,
                method,
            });
